...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      //    上の3条件が変わったかを確認すること。**
      //
      // 📌 上の引用の「7日保留」は 2026-07-30 時点の記録で、現状ではない。
      //    その後 0087(新規の購入上限)・0088(新規原資の30日換金保留と相殺)・
      //    0077(係争中の報酬凍結)を入れた。現在地は docs/chargeback-defense.md。
      //    それでも**登録30日を過ぎた利用者の購入を原資とする分**には
      //    120日の窓が残る。3DSはそこにも効く唯一の手なので、外さないこと。
