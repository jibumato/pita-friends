...
--
-- ⚠️ 0030_refund_lot_expiry.sql は、意図的にこのファイルに含めていません。
--    返金コインの有効期限の扱い(資金決済法の適用除外)について弁護士Q18の回答を
--    待っている段階で、まだ適用してはいけないためです。回答を受けて適用したあとに、
--    このファイルへ取り込んでヘッダの番号を更新してください。
--    詳細: docs/legal/lawyer-review-answers-round2-draft.md
...
--
-- ⚠️ 【要修正・法務確認中】当初「各ロットが6か月未満なら適用除外の趣旨に
--    反しない」と整理していたが、これは誤り。資金決済法4条2号の基準は
--    「発行の日から6月内」であり、購入から5か月後にキャンセルすると
--    当初発行日からの通算で約11か月使えてしまう。
--    消費したロットの期限を記録し、返金時は当初の期限のまま戻すよう
--    改める必要がある。詳細と修正案:
--    docs/legal/lawyer-review-answers-round2-draft.md の「未対応の実装課題」
--    (弁護士Q18の回答を待って着手する)
-- ------------------------------------------------------------
