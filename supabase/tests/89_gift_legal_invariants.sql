...
--
-- ============================================================
-- ⚠️ **このテストを消さないこと。落ちたら実装を直す。**
-- ------------------------------------------------------------
-- 弁護士の指示(2026-07-30 Q24)により、法的根拠をここに明記しておく。
-- 「将来『なぜ落ちるのか分からないテスト』として消される」のを防ぐため。
--
--   ・完了予約の限定  → **資金移動業(為替取引)の該当性回避。**
--                       ギフトを役務に付随する謝礼と性格づける最重要の防壁
--   ・7日換金保留     → マネー・ローンダリング / クレジットカード現金化の対策
--                       (規約 第7条の2第5号。0063で消えていたのを0069で復活)
--   ・報酬コインの非失効 → 労働対価の没収を作らない
--                       (消費者契約法10条・公序。報酬コインは前払式支払手段ではない)
--   ・相互送金の禁止  → 通謀による資金の循環の防止
--
-- **緩める判断は弁護士に相談してから。** 規約に「緩める場合は別途検討を要する」と
-- 書いてはいけない(弁護士Q24: 利用者が緩和を期待する外観を作るため)。
-- 制約の一覧は docs/open-issues.md の🔒欄にある。
-- ============================================================
