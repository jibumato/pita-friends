/**
 * PC(デスクトップ/タブレット)向けランディングページ。
 * スマホ実機幅では表示されず、App 側で 640px 超のときだけ使う。
 * ヒーロー右側に実アプリ(PhoneFrame)を埋め込み、その周りにPCゲーマー向けの
 * 訴求(特長・遊び方・安全・ホスト募集)を置く。文言は「ゲーム仲間」で統一し、
 * 出会い系ではない一線を守る。
 */
...
const FEATURES = [
  {
    icon: '🎮',
    title: '一緒に遊ぶ時間を予約',
    body: '上手い人・気の合う人と、30分から。コインで完結するので、外部で連絡先を交換する必要はありません。',
  },
  {
    icon: '🛡️',
    title: '女性も安心の設計',
    body: '誰から誘いを受けるか自分でコントロール。承認制・ワンタップ通報/ブロック・本人確認・みまもり付き。',
  },
  {
    icon: '🏆',
    title: '実力がちゃんと報われる',
    body: 'プレイ実績・評価・信頼性でランキング。投げ銭や課金額ではありません。ホストは遊んだ時間が報酬コインに。',
  },
]
...
export default function LandingDesktop({ flow, device }: { flow: Flow; device: ReactNode }) {
  const start = () => {
    flow.go(isBackendConfigured ? 'signUp' : 'consent')
    document.getElementById('lp-device')?.scrollIntoView({ behavior: 'smooth', block: 'center' })
  }

  return (
    <div
      className="lp-wrap"
      style={{ width: '100%', minHeight: '100vh', background: C.surface, color: C.ink }}
    >
      {/* ===== ナビ ===== */}
      <header
        style={{
      ...
      >
        ...
        >
          ...
          <button className="lp-cta" onClick={start} style={ctaPrimary}>
            ▶ はじめる
          </button>
        </div>
      </header>
      ...
      <section
        style={{
          background: `radial-gradient(1200px 500px at 80% -10%, ${C.surfaceLavender}, transparent 60%)`,
        }}
      >
          ...
          <div className="lp-hero">
            <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <button className="lp-cta" onClick={start} style={{ ...ctaPrimary, padding: '14px 26px', fontSize: 15 }}>
                  ▶ はじめる（無料）
                </button>
                <button
                  className="lp-cta"
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginTop: 4 }}>
                ...
                <div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}>
                  {PC_GAMES.map((g) => (
                    <span key={g} style={gameBadge}>{g}</span>
                  ))}
            ...
            {/* 実アプリ */}
            <div id="lp-device" style={{ display: 'flex', justifyContent: 'center' }}>
              {device}
            </div>
          </div>
        </div>
      ...
      <section id="safety" style={sectionWrap}>
        <div
          style={{
            ...
            boxShadow: `8px 8px 0 ${C.lavender}`,
          }}
        >
      ...
      <section id="host" style={{ ...sectionWrap, background: C.surfaceLavender }}>
        ...
        >
          ...
          <button className="lp-cta" onClick={start} style={{ ...ctaPrimary, padding: '15px 28px', fontSize: 15 }}>
            ホストとして始める ▶
          </button>
        </div>
      </section>
          ...
          <div style={{ display: 'flex', gap: 18, flexWrap: 'wrap' }}>
            {([
              ...
              <span
                ...
                style={{ cursor: 'pointer', fontSize: 12.5, color: '#E3DCFF', textDecoration: 'underline' }}
              >
                {label}
              </span>
            ))}
          </div>
          <span style={{ fontSize: 11.5, color: '#B3ABC9' }}>
