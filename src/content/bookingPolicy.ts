...
/**
 * 申し込みを押したあと、何が起きるか(0117)。
 *
 * ■ なぜ押す前に出すか
 *   承諾を待つ画面には同じことが書いてあるが、**押したあとに読むのと、
 *   押す前に読むのとでは別物**。初めての人が止まるのは押す直前の一瞬で、
 *   そこで「このあと何が起きるか分からない」と思うと押さない。
 *
 * ■ キャンセル規定とは別に置く
 *   あちらは同意の証跡として記録する文面で、**1文字変えたら版を上げる**
 *   もの。こちらは案内なので、記録しないし版も持たない。
 *   混ぜると、案内の言い回しを直すたびに版が上がる。
 */
export const NEXT_STEPS: string[] = [
  'コインを確保して、ピタメイトに申し込みが届きます（この時点では成立していません）',
  '承諾されると予約成立。トークが開いて、待ち合わせの相談ができます',
  '時間になったら、決めた場所（ゲーム内・Discordなど）で合流します',
  '遊び終わったら「完了」。お互いにレビューを送れます',
]
