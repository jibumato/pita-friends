...
const ALLOWLIST: RegExp[] = [
  ...
  /フレコ/gi,
  /ゲーム\s*(内|の)?\s*ID/gi,
  /(スイッチ|switch|ps[45n]|steam|discord|ディスコ|ディスコード)\s*(の)?\s*(フレンド|ID|名|タグ)/gi,
...
const CONTACT_PATTERNS: RegExp[] = [
  /(ライン|らいん|LINE)\s*(交換|こうかん|して|しよ|やってる|ID|アカ)/gi,
  /\bline\s*(id|add)\b/gi,
...
export function guardWarningText(hits: GuardHit[]): string {
  ...
  return `${reasons}に関わる内容が含まれているようです。ピタフレはゲームを一緒に楽しむ場です。連絡先の交換や金銭のやり取りはアプリ内で完結させてください。`
}
