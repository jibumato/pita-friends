...
## 1-b. 参考：Discord のショップ実物（2026-08 時点）

実際の画面を見て測った値。**目標の見た目はこの水準**。

| 商品 | 構造 | アバターに対する寸法 |
|---|---|---|
| ソーラーパルス | 光るリング1本（日食のような輪郭光） | 約 **1.5倍** |
| アストロラーベ | 細いリングが複数、絡んで回る | 約 **1.8〜2.0倍** |
| クリスタルクラウン | **上に乗る**冠＋左右に垂れる飾り | 横 1.6倍 / **上へ 0.5倍** |
| サンキャッチャー | プロフィール上部を横切る吊り飾り | 帯状（枠の外） |

### ここから読み取れること

1. **装飾はアバターの 1.5〜2.0 倍に収まっている。** 大きく広げていない
2. **「上に乗せる」型が主力**（クリスタルクラウン）。左右へ大きく張り出す
   ものは少ない。**本書の「上のほうが安全」という結論と一致している**
3. **同じ素材の色違いで商品を増やしている**（アストロラーベに金/銀の切替が付いている）
4. **単品 ¥790 / バンドル ¥1,610（-32%）** という値付け
5. 光り物は**細い線＋発光**でできている。塗りの面積は小さい

### ⚠️ そのまま真似できない点：Discord はダークテーマ専用

見本が金色で映えているのは**背景が濃いから**。
**ピタフレはライトとダークの両方**があり、白背景では金や淡い光がほとんど見えない。

- 明度の低い**輪郭線（`#453D5C`）を必ず入れる**。線があれば白背景でも形が残る
- 発光だけに頼らない。**形で見せる**
- **納品前に、白背景と濃紫背景の両方に置いて確認する**（チェックリスト参照）

### 値付けの参考

Discord の ¥790 は、月間2.6億人規模のサービスの値付け。
ピタフレは**単品 300〜500円、4種セットで 1,000〜1,500円**あたりが現実的と考える。
季節4種をまとめたバンドルは、Discord と同じく **3割引** が目安。
...
| 1テーマあたり | パーツ **3〜6個** |
| **装飾の寸法** | アバターの **1.5〜2.0倍**に収める（Discord実物と同じ水準。§1-b） |
...
- [ ] **白背景と濃紫背景の両方で見えるか** ← Discordはダーク専用だが、ピタフレは両対応
...
さがす画面は50pxのアバターが行の左端にあり、すぐ右にテキストが来る。
`overflow: hidden` も効いている（オンラインの緑点だけが例外的に外へ出ている）。
**一覧で左右に張り出す装飾は入らない。**

そして**左右より上のほうが安全**。左右はテキストや隣とぶつかるが、上はカード内の
余白がある。Discord のクリスタルクラウンが「上に乗せる」型なのも同じ理由と思われる。

| パーツ | 出る方向 | 一覧 |
|---|---|---|
| 光輪 / 冠 / 角 | 上 | **○ 使える**（上へ 0.35〜0.5倍） |
| リング（アバターを囲む） | 全周・近い | **○ 使える**（1.5倍まで） |
| 羽 / 翼 | 左右 | **✕ 詳細とトークのみ** |
| 舞う粒（春夏秋冬） | 全方向・小さい | **○ 使える** |

**天使＝光輪、悪魔＝角**を一覧でも見える印にして、**羽と翼は詳細画面のごほうび**に
する。買った人が「詳細を開かせたくなる」ので、動線としても働く。

### 羽1枚のキャンバス指定

片翼だけを作り、反対側は `scaleX(-1)` で反転する（2枚作ると左右非対称になる）。
...
- **根元（付け根）を画像の内側の辺に寄せる。** 中央に置くとアバターから浮く
- アバターの一辺を 1.0 として、**片翼の高さ 1.15 / 外へ出る幅 0.55**
- 羽を含めた全体は横 **2.1倍**。詳細画面（80px → 168px）なら入る

### 色バリエーションで商品を増やす

Discord のアストロラーベには金/銀の切替が付いている。**同じ素材の色違い**なので、
制作コストはほぼゼロで商品が増える。ピタフレでも使える手。

実装は CSS の `filter: hue-rotate()` か、色違いのWebPを別に持つ。
**輪郭線の色は変えない**（`#453D5C` 固定。ここを変えるとアプリから浮く）。
...
| 第2弾 | 😇天使 / 😈悪魔 | 光輪・角は一覧でも使える。羽と翼は詳細画面のみ。羽ばたき・炎の揺らぎだけ Lottie を検討 |

値付けは §1-b のとおり、**単品 300〜500円 / 4種セットは3割引**を目安にする。
