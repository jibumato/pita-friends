...
1. ◑ **マイグレーション適用**: 0001〜0052は適用済み(2026-07-27)。
   **`0053`〜`0056` が未適用**です。0053 → 0054 → 0055 → 0056 の順に当ててください
   (0054は0053の `favorites` を、0056は0053の `my_favorites` を作り直すため、
   逆順では失敗します)。
   ...
   <details><summary>適用済みの内容(0037〜0052)と、次に当てる0053〜0056</summary>
   ...
   | `0053_favorites.sql` ← **次はこれ** | 推し登録(お気に入り) | ⭐を押しても保存されず、ホームの「推しのピタメイト」も出ない |
   | `0054_favorite_slot_notify.sql` ← **その次** | 推しているピタメイトが枠を開けたら通知(24時間に1回まで・増えた分のみ) | 枠を開けてもファンに伝わらず、空いた枠が埋まらない |
   | `0055_play_history_with.sql` ← **その次** | 「あなたとは3回遊んでいます」の回数 | プロフィールに二人の実績が出ない(通算の回数だけになる) |
   | `0056_host_status.sql` ← **その次** | ピタメイトの「ひとこと」(近況・60字・14日で自動的に非表示) | ひとことを書く欄が出ず、ホームに更新されるものが無いまま |
