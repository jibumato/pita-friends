...
1. ✅ **マイグレーション適用**: **0001〜0095 まで適用済み**
   (0083までを2026-07-31、`0084`〜`0095` を2026-08-03。`0065` は
   `0093`/`0094` が内容を含むため当てない)。
   ...
   - ☐ **`0095` を当てる前にコインの付与関数が開いていた**ので、
     `coin_purchases` に身に覚えのない行が無いことを一度だけ確認する
   ...
   → `0095_close_server_only_functions`(**Edge Function 専用の関数を利用者から閉じる。コインの付与が開いていた**)
...
5. ✅ **(必須)Pro プランにする**($25/月。無料プランには保証されたバックアップが無い)
   ...
   - ☐ PITR($100/月 + Small コンピュート$15/月)は**未契約と確認済み**（2026-09-26）。取引量が増えてからで可
...
> ### ✅ テストモードでの一気通貫確認は完了(2026-08-04)
>
> サンドボックス「ピタフレ」で 5,000コインパックを購入し、次を実測で確認済み。
>
> | 見たもの | 結果 |
> |---|---|
> | Checkout の明細 | **2行**(コイン ¥5,000 ＋ あんしんサポート料 ¥250) |
> | Stripe の総売上高 | **¥5,250** |
> | Webhook `checkout.session.completed` | **200 OK**・本文 `"ok"`(= 関数の最後まで到達) |
> | ウォレット残高 | **5,000コイン** |
> | 有効期限 | 2027/2/3(取得日から6か月未満。資金決済法4条2号の適用除外) |
> | `coin_purchases` | 1行のみ。想定外の付与なし |
>
> **⚠️ Stripeアカウントは、運営中の別サイトとは分けてあります。**
> ピタフレは「見知らぬ人同士・前払いのコイン・お金が動く」構成で、Stripeの
> リスク審査で最も突かれやすい部類です。同居させると、こちらで
> チャージバックが立て込んだときに**別サイトの入金まで止まります。**
> 本番化のときも、必ずピタフレ側のアカウントで作業してください。
>
> **決済手数料の実測メモ**: テストでは ¥5,250 に対し ¥208(約4%)。
> つまり**あんしんサポート料 ¥250 は、その取引の決済手数料でほぼ相殺される。**
> 収益はホスト手数料から出るので設計どおりだが、「コインは売れたが誰も
> 遊ばない」状態が続くと、手数料だけ出て預り金が積み上がる。
> (テストモードの手数料は実契約の料率とは限らない。本番1件目で実額を見ること)
...
5. ☐ 本番カードで少額(¥500・最小パック)の実購入テスト → コイン付与を確認 → 必要なら返金
...
- ☐ **Supabase の PITR が有効になっている**(取引データの復旧手段)。
  **2026-09-26 時点で未契約と確認済み**（Proのみ有効化）。B.5参照
