...
1. ⏳ **マイグレーション適用**: 0001〜0083は適用済み(2026-07-31)。
   **`0084`〜`0092` が未適用です。**
   `0084` は設定画面の通知トグルが反応しない不具合の修正、
   `0085` は規約第9条5の3(無帰責の返還で消滅した分の金銭返金)、
   `0086` は規約第6条の2(退会)、`0087` は規約第8条の6第5項1号
   (新規ユーザーの購入上限)、`0088` は同条第3〜5項(チャージバック清算の
   相殺と換金保留)、`0089` は第7条5の3・第9条4項(失効の事前通知と申出の
   14日制限)の実装です。**すべて公開前に当ててください。**
   `0089` は定期ジョブ `notify-expiring-coins` を足すので、適用後に
   `docs/check-cron.sql` で**11件**そろっているか確認してください。
   `0090` は購入の取消し(あんしんサポート料の返還)で、運営コンソールの
   「返金」タブから操作します。`0091` は料率の変更を運営コンソールの
   「料率」タブに寄せ、**30日以上先でしか変更できない**ようにします。
   **`0087` を当てたら `create-checkout-session` の再デプロイも必要です**
   (上限の判定は Edge Function から呼びます)。
   `0086` は新しい定期ジョブ(`expire-withdrawn-earned`)を登録するので、
   適用後に `docs/check-cron.sql` で10件そろっているか確認してください。
