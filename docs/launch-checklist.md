...
1. ◑ **マイグレーション適用**: 0001〜0056は適用済み(2026-07-27)。
   **0057〜0078が未適用**です。次の順に当ててください:
   ...
   <details><summary>適用済みの内容(0037〜0056)と、次に当てる0057〜0078</summary>
   ...
   | `0064_web_push.sql` ← **その次** | プッシュ通知（購読先・送信待ち・静かにする時間。**別途VAPID鍵の設定が必要** → `docs/web-push-setup.md`） | 通知がアプリを開いたときしか見えないまま（0054の枠空き通知が届かない） |
   | `0065_lock_down_function_grants.sql` ← **その次** | 未ログインに開いていた関数を閉じる（**他人の予約表が読める / 監査台帳に嘘を書ける** の2件を実際に再現して修正。`docs/security-review.md`） | **公開してはいけない状態のまま**になる |
   | `0066_admin_console.sql` ← **その次** | 運営コンソール（保留の解除・通報の審査・換金・請求・健全性・操作記録。`docs/admin-console.md`） | 締切のある作業を毎回 SQL Editor で手打ちすることになる |
| `0067_favorites_wording.sql` ← **その次** | 「推し」→「お気に入り」の言い換えを説明文にも反映（表・関数は変わらない） | 開示請求への回答やDBの説明文に古い言い方が残る |
| `0068_admin_read_audit.sql` ← **その次** | 本人確認の承認/却下と通報の閲覧を操作記録に残す（弁護士指摘7） | **メッセージの中身を見た記録が残らない** |
| `0069_restore_gift_payout_hold.sql` ← **その次** | ギフト受領から7日間は換金できない保留を復活（`0063` が誤って消していた。規約第7条の2第5号） | **規約に書いてある保留が効かない**（AML対策の穴） |
| `0070_accounting_reconciliation.sql` ← **その次** | 会計用の残高・売上の集計（前受金/預り金/未払金、税区分つき。税理士の勘定設計に対応） | 決算のたびに手で集計することになる |
...
| `0075_payment_dispute_freeze.sql` ← **その次** | チャージバック（決済の異議申立て）を受けたら、その方のコインの利用を止める。すでに成立した予約の進行は止めない | **「異議を申し立てながら、その間にコインを使い切る」が通ります。**税理士が「リリース前の必須実装・優先度は最上位」とした項目 |
| `0076_accounting_round2.sql` ← **その次** | 会計集計の科目を税理士の第2回指摘に合わせる（「未払金」→預り金の下位／**仮受金(換金手数料)**の追加／分別対象額の合計／フロー4項目） | 換金申請中の**手数料300コインがどの勘定にも現れず**、分別管理で守るべき額がずれる |
| `0077_dispute_payout_hold.sql` ← **その次** | チャージバックの係争中、**その決済に紐づく予約の報酬だけ**を換金保留にする（ピタメイト全体は止めない） | **係争中に当該取引の報酬が換金されて出ていき、回収不能が確定する**。0069の `request_bank_payout` を作り直すので順番を飛ばさないこと |
| `0078_bonus_origin_fee.sql` ← **最後** | 「PF利用料のうち無償コイン起因」を内数で出す（純額処理を選ぶための材料） | 両建て処理しかできず、**現金を受け取っていない取引で課税売上高が膨らむ**。1,000万円の判定が実態より早く来る |
