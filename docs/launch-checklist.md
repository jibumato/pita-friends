...
- ☐ **`src/launchPhase.ts` の `IS_PRE_LAUNCH` を `false` にした**(下記)
- ☐ 事前登録者へ公開のお知らせを送った(下記)

### 公開スイッチ(`src/launchPhase.ts`)

公開前はアカウント登録の導線を隠し、事前登録に一本化しています。
コイン購入も換金もできない状態でアカウントだけ増えると、本人確認の審査だけが
発生してしまうためです。**ログインは塞いでいません**(動作確認のため)。

公開するときは `IS_PRE_LAUNCH` を `false` にして main にマージするだけです。
これを戻し忘れると、**公開したのに誰も登録できない**状態になります。

| `IS_PRE_LAUNCH` | ランディング | スマホのようこそ画面 |
|---|---|---|
| `true`(公開前) | 事前登録フォームのみ。ヘッダーCTAはフォームへスクロール | 「公開のお知らせを受け取る」+ 近日公開バッジ |
| `false`(公開後) | 「▶ はじめる（無料）」が戻る | 「▶ PRESS START」「新規登録」が戻る |
