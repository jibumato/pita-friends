...
1. ◑ **マイグレーション適用**: 0001〜0056は適用済み(2026-07-27)。
   **0057〜0077が未適用**です。次の順に当ててください:
   ...
   <details><summary>適用済みの内容(0037〜0056)と、次に当てる0057〜0077</summary>
...
| `0077_dispute_payout_hold.sql` ← **最後** | チャージバックの係争中、**その決済に紐づく予約の報酬だけ**を換金保留にする（ピタメイト全体は止めない） | **係争中に当該取引の報酬が換金されて出ていき、回収不能が確定する**。0069の `request_bank_payout` を作り直すので順番を飛ばさないこと |
