...
jobs(seq, jobname, schedule, purpose, blocker) as (values
  ...
  (13, 'expire-guest-requests',         '*/15 * * * *',
      ...
      'ピタメイトの意図しない時間に予約が入る'),
  (14, 'flag-dormant-accounts',        '0 4 * * *',
      '最終ログイン2年以上のアカウントに30日前通知(規約第6条の4・0127)',
      '★規約に書いた通知義務を果たせない。長期未ログインのまま放置される'),
  (15, 'execute-dormant-accounts',     '20 4 * * *',
      '通知から30日後、まだ休眠なら利用を停止する(規約第6条の4・0127)',
      '通知は届いても、その後の措置(停止)が実行されないまま残る')
),
