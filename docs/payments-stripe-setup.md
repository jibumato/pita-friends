...
そのまま書きます）。不正検知も、カードのフィンガープリントが取れない手段を
「判定できない購入」として別に数える形に既になっています（`0096`）。

見るべき論点は3つあり、**そのうち1つは解消済みです。**

| 論点 | 状態 |
|---|---|
| **料率** | ✅ **問題なし。** WeChat Pay・Alipay もカードと同じ 3.6%（2026-08-26 確認）。分別管理規程 第7条1号の試算「サポート料5% ＞ 実質負担3.78%」はそのまま成り立ち、**補填義務は発動しません**。※料率の違う手段を足すときは、ここを再計算すること |
| **3DS** | 要確認。`payment_method_options.card` は**カードにしか効きません**（Apple Pay / Google Pay / Link は実質カードなので対象）。ただし **Alipay・WeChat Pay はそもそも異議申立ての扱いがカードと違う可能性が高い**ので、「3DSが無い＝危険」とは限りません。Stripe のダッシュボードで、その手段に disputes があるかを確認してください。**無いなら、むしろカードより安全側です** |
| **居住地** | ⚠️ 残る判断。規約 第3条3項は「日本国内に居住する個人に限る」で、`0081` が本人確認の提出を止めます。**WeChat Pay・Alipay は中国系のウォレット**で、その前提と外形が噛み合いません（在日の方が持っていることは普通にあるので、非居住の証拠にはなりません）。**Stripe の本番申請では事業内容と決済手段の整合を見られることがあります** |

### どうするか

料率が同じである以上、**残っているのは居住地の外形だけ**で、技術的な問題は
ありません。判断は次の2つです。

- **絞る** … 日本国内居住者限定という建てつけと、決済手段の見た目を揃える
- **そのまま** … 選択肢が多いほうが購入は通りやすい。在日の利用者にも実際に使われる

どちらを選ぶにせよ、**環境変数で明示してください。** 未設定のままだと、
ダッシュボードを触ったときに決済手段が黙って変わります。

```bash
# 絞る場合
supabase secrets set STRIPE_PAYMENT_METHODS=card,paypay
# そのまま使う場合（いま出ているものを明示する）
supabase secrets set STRIPE_PAYMENT_METHODS=card,paypay,wechat_pay,alipay
supabase functions deploy create-checkout-session
```

> ⚠️ **ここに書いた手段を Stripe 側で有効化していないと、セッション作成が
> 失敗します**（購入ボタンでエラーになる）。名前は Stripe の
> `payment_method_types` の表記に合わせること。
