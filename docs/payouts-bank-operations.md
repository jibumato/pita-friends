...
### 振込元口座: GMOあおぞらネット銀行(2026-07 開設手続中)

- GMOあおぞらネット銀行の**総合振込**は、全銀協規定フォーマット(固定長テキスト)の
  アップロードに対応しています。上記CSVの列は銀行側の画面/テンプレートに合わせて
  並べ替えてください(**列順・ヘッダ有無・文字コードは実際の管理画面で要確認**)。
- **受取人名は半角カナ・最大30文字**です(全銀の受取人名欄の上限)。アプリ側は
  カナ48文字まで許容しているので、30文字を超える名義は銀行のアップロード時に
  弾かれます。該当したら `mark_payout_failed` で差し戻し、名義の短縮を依頼します。
...
それでも記号・番号のまま登録してくる人は出ます。**9900 の行は振込前に目視で
確認**し、口座番号が変換後の形(7桁・末尾1が付いていない等)になっているかを見ること。
怪しい行は振り込まず、本人に確認してから実行します。
