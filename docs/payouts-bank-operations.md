...
| 銀行 | 状態 |
|---|---|
| PayPay銀行 ビジネスアカウント | **開設済み**（屋号 Type&Co で通った） |
...
#### 口座名義照会(受取人名確認)の使い方 ★ロックに注意

PayPay銀行には受取人名確認があります。**ただし方式によって効き方が違います。**

| 方式 | 名義確認 |
|---|---|
| 通常振込・**WEB一括振込** | 金融機関・支店・口座番号を入れると、**正しい名義(カナ)が自動表示される** |
| WEB総振 | **CSVに正しい名義を書いておく必要がある**（自動表示されない） |

★**これは WEB一括振込を選ぶ、もう一つの理由です。** 振込先を事前登録する
ときに名義が自動で出るので、ピタメイトの自己申告と食い違っていればその場で
気づけます。手入力も要りません。

⚠️ **照会だけを繰り返すとロックがかかります。**
振込を完了させずに名義照会を何度も繰り返すと、不正利用防止で機能が止まります。

  → **口座登録のたびに照会をかける運用にしないこと。**
    ピタメイトが口座を登録した時点では照会せず、**換金申請が出て、
    実際に振り込む段になってから**、振込先の登録と同時に確認します。
  → 週次の締めでまとめて登録するときも、件数が多い週は分けてください。

⚠️ 次の場合は名義が表示されず、手入力になります。

  ・受取人名確認の取扱時間外(メンテナンス等)
  ・振込先の金融機関が夜間・土日の照会に対応していない
  ・相手方が口座名義の開示拒否設定をしている

  → **金曜の日中に作業する**理由がここにもあります。夜間や土日に回すと
    照会が使えず、手入力＝打ち間違いのリスクが上がります。

照会を飛ばして振込後に返る(組戻し)と、手数料¥660〜880が当社負担です
(§②末尾の💡も参照)。
