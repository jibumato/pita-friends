...
ログイン画面の「パスワードをお忘れですか？」から、再設定用のリンクをメールで
送ります。実装は Supabase Auth の標準機能をそのまま使っています。

```
ログイン画面 → メールアドレスを入力 → resetPasswordForEmail()
  ↓ メールのリンク
https://pitafure.com/reset-password#access_token=...&type=recovery
  ↓ Supabase JS がハッシュから「復旧セッション」を復元
アプリが /reset-password を見て、再設定画面に固定する
  ↓ 新しいパスワードを入力 → updateUser({ password })
他の端末のセッションを切る → サインアウト → ログインし直してもらう
```

### 気をつけている点

**復旧セッションのままホームに入れない。** リンクを開いた時点で Supabase は
セッションを作るので、何もしないと**パスワードを知らない人がログイン済みとして
アプリに入れてしまいます**。`App.tsx` の起動処理でパスを見て再設定画面に固定し、
`PASSWORD_RECOVERY` イベントでも拾う二重の構えにしています。

**アカウントの有無を漏らさない。** 未登録のアドレスを入れても
「登録があればお送りしました」と同じ表示にします。ここで「登録がありません」と
出すと、他人のアドレスが登録済みかを調べる手段になります。

**設定後に他の端末を切る。** パスワードを忘れる状況には「乗っ取られたので
変えたい」も含まれます。変えたのに相手のセッションが生き続けるのは危険なので、
`signOut({ scope: 'others' })` で落とします(コインと換金を扱うので特に)。

**退会済みの人は再設定できません。** `anonymize_user()`(0046)がメールアドレスを
`anonymized+<uuid>@invalid.local` に置き換えるため、元のアドレスでは一致しません。

### リンクの有効期限が切れていたら

「リンクの有効期限が切れています。もう一度メールを送ってください。」と出して、
ログイン画面からやり直してもらいます。既定の有効期限は1時間です
(Authentication → Sessions で変更可)。
