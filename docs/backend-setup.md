...
**注意**: 認証は実接続されますが、**画面に表示されるコイン残高・マナースコア・プロフィール等のデータはまだローカルのモック状態のままです**（Supabase上に作成された実際の行を読み書きするようにはなっていません）。サインアップすると `handle_new_user` トリガーで実際にDB行（`profiles` / `coin_wallets` 等）が作られますが、アプリのUIはまだそれらを参照していません。この配線は次のステップです。

## まだ設計・実装していないもの

- **プロフィール・コイン残高等の実データ取得** — 上記のとおり、UIはまだモック状態を表示している
